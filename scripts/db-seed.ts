/**
 * Bootstrap admin (Better Auth password hash) + sample vulnerabilities from fixtures.
...
async function ensureBootstrapAdmin(): Promise<string> {
  const email =
    process.env.BOOTSTRAP_ADMIN_EMAIL?.trim().toLowerCase() || "admin@local.dev";
  ...
  if (existingAdmins.length > 0) {
    console.log(`[bootstrap] admin already exists (${existingAdmins[0].email})`);
  ...
  if (existingByEmail.length > 0) {
    console.log(`[bootstrap] using existing user ${email}`);
...
async function main() {
  ...
  const ownerId = await ensureBootstrapAdmin();
